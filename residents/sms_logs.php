...
$resident_id = $_SESSION['resident_id'];  // Retrieve the resident's ID from session
...
// Fetch SMS records from tbl_sms_resi
$sql = "SELECT contact, message, send_time AS sms_time FROM tbl_sms_resi WHERE resident_id = ? ORDER BY send_time DESC";  // Added 'resident_id' condition
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $resident_id);  // Bind resident_id to the query
$stmt->execute();
...
<div class="table-responsive">
    <table class="table table-striped table-bordered">
        ...
        <tbody>
            <?php
            if (count($smsRecords) > 0) {
                foreach ($smsRecords as $row) {
                    echo "<tr>";
