...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Query to fetch the current password for the official
    $sql = "SELECT password FROM tbl_off WHERE id = '$official_id'";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        $official = $result->fetch_assoc();
        $hashed_old_password = $official['password']; // Retrieve stored hashed password
        ...
        if (password_verify($old_password, $hashed_old_password)) {
            ...
            if ($new_password == $confirm_password) {
                ...
                $update_sql = "UPDATE tbl_off SET password = '$hashed_new_password' WHERE id = '$official_id'";
